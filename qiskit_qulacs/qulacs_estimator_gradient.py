...
from qiskit.algorithms.gradients.estimator_gradient_result import (
    ...
    def _run(
        ...
        **options,
        ...
        for circuit, observable, parameter_values_, parameters_ in zip(
            ...
            qulacs_circuit, metadata = convert_qiskit_to_qulacs_circuit(
                circuit
            )
            ...
            gradient = np.negative(
                qulacs_circuit(np.array(parameter_values_)).backprop(
                    qulacs_obs
                )
            )

            gradient[parameter_mapping] = gradient[
                np.arange(len(parameter_mapping))
            ]
        ...
        return EstimatorGradientResult(
            gradients=gradients, metadata={}, options=opt
        )
